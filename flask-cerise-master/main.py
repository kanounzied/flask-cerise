...
def vie2():
    ...
    if request.method == 'POST' and session['vie1']:
        ...
        if session['age'].isnumeric() and int(session['age']) > 0:
            ...
            erreur = {'fr': "L'age doit être une valeur numérique non vide", 'en': "Enter a non void number",
                      'ar': "أدخل رقما"}
    if not (session['vie1']):
        return redirect(url_for("vie1"))
...
def vie3():
    ...
    if request.method == 'POST' and session['vie22']:
        ...
        if session['w'].isnumeric() and 15 < int(session['w']) < 300 and session['h'].isnumeric() and 15 < int(
            ...
            error = {'fr': "Taper votre taille et votre poids", 'en': 'Provide height and wieght',
                     'ar': 'أدخل طولك و وزنك'}
